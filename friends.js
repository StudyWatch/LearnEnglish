...
function displayTips() {
    const tips = [
        "<strong>רשימת מילים:</strong> לפני כל פרק, עבור על רשימת המילים שאתה עתיד ללמוד. זה יעזור לך להכיר ולזהות את המילים במהלך הצפייה.",
        "<strong>צפייה פעילה:</strong> כשאתה צופה בסדרה, שים לב במיוחד למילים וביטויים שלמדת. נסה להיות ממוקד ולהבין איך הם משולבים בדיאלוג.",
        "<strong>חזרה על המילים:</strong> לאחר סיום הפרק, חזור על רשימת המילים. נסה לכתוב משפט אחד לפחות עם כל מילה לשילוב הטכניקה שלמדת.",
        "<strong>שימוש בתרגומים:</strong> אם יש מילה שאתה לא מבין, עזור לעצמך במילון או בתרגום כדי להבין את משמעותה המדויקת.",
        "<strong>צפייה מחודשת:</strong> צפה שוב בפרקים שכבר ראית, כעת נסה להבין את התוכן והשפה ללא תרגום. זה מאפשר תרגול יעיל יותר של ההבנה.",
        "<strong>תרגול עם חברים:</strong> שוחח על הסדרות והמילים שלמדת עם חברים שלומדים גם הם אנגלית. זה ישפר את הדיבור והבנת השפה.",
        "<strong>הקלט והאזן:</strong> הקלט את עצמך מדבר באנגלית ואז האזן להקלטה. זה יסייע לך לזהות איזו תיקון נדרש בהגייה ובשימוש בשפה.",
        "<strong>שימוש ביומן:</strong> רשום ביומן את התובנות והשאלות שלך לגבי הלמידה. זה יעזור לך לעקוב אחרי ההתקדמות ולהתמיד בלמידה.",
        "<strong>חשיבה באנגלית:</strong> נסה לחשוב על התוכן שראית באנגלית. זה יכול להיות מאתגר אך יעזור לך להפנים את השפה באופן יותר טבעי.",
        "<strong>הכנת תקצירים:</strong> אחרי הצפייה בפרק, כתוב תקציר קצר באנגלית של העלילה והשימושים שזיהית במילים שלמדת. זה יעזור לך לחזק את הידע ולזכור את המילים בהקשרן.",
        "<strong>משחקים אינטראקטיביים:</strong> השתמש במשחקים האינטראקטיביים שהאתר מציע לאחר כל פרק כדי לבחון את הבנתך וזכירתך של המילים. המשחקים מותאמים לרמתך ויעזרו לך לחזק את הידע בצורה מהנה.",
        "<strong>צפייה עם כתוביות:</strong> נסה לצפות בפרקים עם כתוביות באנגלית. זה יכול לעזור לך לשפר את ההבנה ואת יכולת הקריאה שלך באנגלית.",
        "<strong>שימוש בכרטיסיות:</strong> צור כרטיסיות לכל מילה חדשה עם דוגמאות מהקונטקסט של הסדרה, וסקור אותן באופן קבוע.",
        "<strong>תרגול כתיבה:</strong> כתוב חיבורים קצרים או יומנים באנגלית, תוך שימוש במילים ובביטויים שלמדת מהסדרות. זה יעזור לך לחזק את המילים החדשות ולשפר את יכולת הכתיבה שלך.",
        "<strong>מעקב אחרי דמויות:</strong> בחר דמות אהובה מהסדרה ונסה לעקוב אחרי השפה והביטויים שהיא משתמשת בהם. זה יעזור לך להבין את הסגנון והקונטקסט שבו משתמשים במילים שונות.",
        "<strong>השוואת גרסאות:</strong> אם יש גרסאות שונות של אותה סדרה (לדוגמה, גרסה בריטית וגרסה אמריקאית), נסה לצפות בשתיהן ולשים לב להבדלים בשפה ובתרבות.",
        "<strong>לימוד תרבותי:</strong> בנוסף ללימוד השפה, נסה להבין את ההקשרים התרבותיים של הסדרה. זה יכול לכלול התנהגויות, מסורות או מבטאים מקומיים.",
        "<strong>צפייה בז'אנרים שונים:</strong> נסה לצפות בסדרות מז'אנרים שונים (דרמה, קומדיה, מתח וכו'). כל ז'אנר יכול להעשיר את אוצר המילים שלך בדרכים שונות.",
        "<strong>ניתוח סצנות:</strong> בחר סצנות מעניינות ונסה לנתח אותן מבחינת השפה, השימוש בביטויים והדיאלוגים. זה יעזור לך להבין לעומק את השימוש בשפה.",
        "<strong>שימוש באפליקציות עזר:</strong> השתמש באפליקציות שמספקות כתוביות, תרגומים וכלים נוספים ללימוד אנגלית. אפליקציות אלו יכולות לסייע בלמידה מעמיקה וממוקדת יותר.",
        "<strong>לימוד באמצעות שירים:</strong> אם הסדרה כוללת פסקול, נסה ללמוד את מילות השירים. שירים יכולים להיות דרך מהנה ויעילה לשיפור השפה.",
        "<strong>שיפור תשומת הלב לשפה:</strong> השימוש באתר שלנו עוזר לשים יותר טוב לשפה האנגלית ולשיפור הדיבור. כאשר צופים בסדרות סתם כך, פעמים רבות מתמקדים בכתוביות ולא בשפה עצמה. האתר שלנו מאפשר לך ללמוד ולקרוא את המילים מראש, מה שמגביר את תשומת הלב לשפה ומוביל לשיפור מהיר יותר."
    ];
